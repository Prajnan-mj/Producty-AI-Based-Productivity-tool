...
const COLORS = { health: "#34D399", learning: "#4F8EF7", work: "#FBBF24", personal: "#A78BFA", career: "#4F8EF7", finance: "#FBBF24" };
...
function Confetti({ show }) {
  ...
  const particles = Array.from({ length: 12 }, (_, i) => ({
    ...
    color: ["#34D399", "#FBBF24", "#4F8EF7", "#A78BFA", "#F87171"][i % 5],
  }));
  return (
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <UrgencyRing percentage={Math.min(weekPct, 100)} size={64} strokeWidth={5}
        color={habit.completed_today ? "#34D399" : habit.color || COLORS[habit.category]} />
...
function CreateHabitModal({ open, onClose }) {
  const qc = useQueryClient();
  const [form, setForm] = useState({ name: "", description: "", frequency: "daily", target_count_per_period: 1, category: "personal", color: "#6366f1", icon: "⭐" });
...
function GoalCard({ goal }) {
  ...
  const trackColor = goal.on_track_status === "on_track" ? "#34D399" : goal.on_track_status === "slightly_behind" ? "#FBBF24" : "#F87171";
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <span className="rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize" style={{ background: (COLORS[goal.category] || "#A78BFA") + "18", color: COLORS[goal.category] || "#A78BFA" }}>
              {goal.category}
            </span>
        ...
        <div className="w-20 h-8">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={sparkData}><Line type="monotone" dataKey="v" stroke="#A78BFA" strokeWidth={1.5} dot={false} /></LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="h-2.5 rounded-full bg-bg-elevated overflow-hidden">
          <motion.div className="h-full rounded-full" initial={{ width: 0 }}
            ...
            style={{ background: `linear-gradient(90deg, #34D399, #A78BFA)` }} />
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="flex flex-wrap gap-1.5">
          {GOAL_CATS.map((c) => (
            <button key={c} onClick={() => setForm({ ...form, category: c })}
              className={`rounded-full px-3 py-1 text-[11px] font-semibold capitalize transition ${form.category === c ? "text-bg-base" : "bg-bg-elevated text-text-muted"}`}
              style={form.category === c ? { background: COLORS[c] || "#A78BFA" } : undefined}>{c}</button>
          ))}
        </div>
...
export default function Habits() {
  ...
  const ringData = summary ? [{ name: "done", value: summary.completion_percentage, fill: "#34D399" }] : [];
